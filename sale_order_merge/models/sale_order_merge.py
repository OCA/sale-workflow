...
class SaleOrderMerge(models.TransientModel):
    ...
    to_merge = fields.Many2many(
        "sale.order", "rel_sale_to_merge", "sale_id", "to_merge_id", "Orders to merge"
    )
    ...
    def merge_order_lines(self):
        self.sale_order.write(
            {
                "order_line": [
                    (4, line.id) for line in self.to_merge.mapped("order_line")
                ]
            }
        )
    ...
    def merge_invoices(self):
        ...
        for invoice in self.sale_order.invoice_ids + self.to_merge.mapped(
            "invoice_ids"
        ):
    ...
    def merge_pickings(self):
        ...
        self.env["procurement.order"].search(
            [("group_id", "in", other_groups.ids)]
        ).write({"group_id": group.id})
        pick_map = {}
    ...
    def merge(self):
        ...
        if not all(order.state in ("sent", "draft") for order in orders):
            ...
            # generation
            if order_policy == "prepaid" and self.sale_order.picking_ids:
                create_picking = True
            if (
                order_policy == "manual"
                and self.sale_order.state == "progress"
                and (drafts or confirmed.filtered(lambda o: o.state == "manual"))
            ):
                reset_wait_invoice = True

            # Propagate order policy across draft orders
            drafts.filtered(lambda o: o.order_policy != order_policy).write(
        ...
        if reset_wait_invoice:
            ...
            if item:
                item_vals = {"act_id": self.env.ref("sale.act_wait_invoice").id}
                if item.subflow_id:
        ...
        self.sale_order.message_post(
            _("Order(s) %s merged into this one")
            % ",".join(self.to_merge.mapped("name"))
        )
