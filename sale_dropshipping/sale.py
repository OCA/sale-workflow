...
class procurement_order(orm.Model):
    ...
    def allow_automatic_merge(
        self, cr, uid, procurement, po_vals, line_vals,
        context=None
    ):
        # template function to determine if the PO generated by the
        # procurement should be automatically merged
        return False
    ...
    def create_procurement_purchase_order(
        ...
        res = super(procurement_order, self).create_procurement_purchase_order(
            cr, uid, procurement,
            po_vals, line_vals, context)

        # Try to merge newly-created PO with existing ones with the same
        # origin, supplier and destination address
        if self.allow_automatic_merge(cr, uid, procurement, po_vals,
                                      line_vals, context=context):
            search_fields = purchase_obj._key_fields_for_grouping() + \
                ('origin',)
            po_matches = purchase_obj.search(
                cr, uid,
                [(field, '=', po_vals.get(field, False))
                 for field in search_fields],
                context=context
            )
            merged_po = purchase_obj.do_merge(cr, uid, po_matches,
                                              context=context)
            # If POs were merged, modify existing procurements with new PO
            if len(merged_po) > 0:
                res = merged_po.keys()[0]
                # Modify other procurements with new PO
                proc_ids = self.search(
                    cr, uid,
                    [('purchase_id', 'in', merged_po[res])], context=context)
                self.write(cr, uid, proc_ids, {'purchase_id': res}, context)
