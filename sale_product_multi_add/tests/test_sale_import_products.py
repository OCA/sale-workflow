# Copyright 2024 maxhe2014
# License AGPL-3.0 or later (http://www.gnu.org/licenses/agpl.html).

import odoo.tests.common as common


class TestSaleImportProducts(common.TransactionCase):
    def setUp(self):
        super().setUp()
    ...
    def test_get_line_values_with_pricelist(self):
        """Test _get_line_values method with pricelist pricing and discount rules"""
        # Create a partner and pricelist
        partner = self.env.ref("base.res_partner_2")
        
        # Create a pricelist with a 10% discount rule
        pricelist = self.env["product.pricelist"].create(
            {
                "name": "Test Pricelist with 10% Discount",
                "currency_id": self.env.ref("base.EUR").id,
            }
        )
        
        # Create a pricelist item with 10% discount
        ...
        # Verify that price is calculated using pricelist (should be discounted)
        # The price should be 90.0 (100 * 0.9) instead of the original 100.0
        self.assertNotEqual(line_values.get("price_unit"), self.product_1.list_price)
        self.assertEqual(line_values.get("price_unit"), 90.0)  # 10% discount applied
    ...
    def test_get_line_values_without_pricelist(self):
        ...
        # Verify that price is the original list price (no discount)
        self.assertEqual(line_values.get("price_unit"), self.product_2.list_price)
    ...
    def test_select_products(self):
        ...
        # Verify the created order lines
        line1 = so.order_line.filtered(lambda l: l.product_id == self.product_1)
        line2 = so.order_line.filtered(lambda l: l.product_id == self.product_2)
    ...
    def test_select_products_empty_items(self):
        ...
        # Check that no sale order lines were created
