...
class sale_stock_prebook(osv.osv_memory):
    ...
    def button_confirm(self, cr, uid, ids, context):
        if context['active_model']=='sale.order':
            ...
            self.pool.get('sale.order.line')._prebook(cr,uid,order_line_ids,context=context)
        elif context['active_model']=='sale.order.line':
            self.pool.get('sale.order.line')._prebook(cr,uid,context['active_ids'],context=context)
