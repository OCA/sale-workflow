...
class TestProgramRules(TestSaleCouponCommon):
    ...
    def test_program_rules_partner_based(self):
        ...
        self.immediate_promotion_program.write(
            {'rule_partners_domain': "[('id', 'in', [%s])]" % (self.steve.id)})
        ...
        self.assertEqual(len(order.order_line.ids), 3,
                         "The promo offert should have been applied as the partner is correct, the discount is not created")
        ...
        self.assertEqual(len(order.order_line.ids), 2,
                         "The promo offert shouldn't have been applied, the discount is created")
    ...
    def test_program_rules_minimum_purchased_amount(self):
        ...
        order.recompute_coupon_lines()
        self.assertEqual(len(order.order_line.ids), 2,
                         "The promo offert shouldn't have been applied as the purchased amount is not enough")
        ...
        # 10*100 + 5 = 1005
        self.assertEqual(len(order.order_line.ids), 2,
                         "The promo offert should not be applied as the purchased amount is not enough")

        self.immediate_promotion_program.rule_minimum_amount = 1005
        order.recompute_coupon_lines()
        self.assertEqual(len(order.order_line.ids), 3,
                         "The promo offert should be applied as the purchased amount is now enough")
        ...
        order.recompute_coupon_lines()
        self.assertEqual(len(order.order_line.ids), 3,
                         "The promo offert should be applied as the initial amount required is now tax included")
    ...
    def test_program_rules_coupon_qty_and_amount_remove_not_eligible(self):
        ...
        self.env['sale.coupon.generate'].with_context(
            active_id=program.id).create({}).generate_coupon()
        ...
        self.assertEqual(len(order.order_line.ids), 3,
                         "The order should contains the Product A line, the Product B line and the discount line")
        self.assertEqual(
            coupon.state, 'used', "The coupon should be set to Consumed as it has been used")
        ...
        self.assertEqual(
            coupon.state, 'new', "The coupon should be reset to Valid as it's reward got removed")
    ...
    def test_program_rules_promotion_use_best(self):
        ...
        self.assertTrue('Get 5% discount' in discounts.pop(),
        ...
        self.assertEqual(
            len(discounts), 1, "The order should contains the Product A line and a discount")
        self.assertTrue('Get 10% discount' in discounts.pop(),
