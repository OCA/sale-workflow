...
class TestSaleCouponProgramNumbers(TestSaleCouponCommon):
    ...
    def test_program_numbers_free_and_paid_product_qty(self):
        ...
        self.assertEqual(len(order.order_line.ids), 2,
                         "We should have 2 lines as we now have one 'Free Large Cabinet' line as we bought 4 of them")
        ...
        self.env['sale.coupon.apply.code'].sudo(
        ).apply_coupon(order, 'test_10pc')
        self.assertEqual(len(order.order_line.ids), 3,
                         "We should 3 lines as we should have a new line for promo code reduction")
        self.assertEqual(order.amount_total, 864,
                         "Only paid product should have their price discounted")
        order.order_line.filtered(
            lambda x: 'Discount' in x.name).unlink()  # Remove Discount
        ...
        self.assertEqual(len(order.order_line.ids), 1,
                         "Free Large Cabinet should have been removed")
        ...
        order.recompute_coupon_lines()
        self.assertEqual(order.order_line.filtered(
            lambda x: x.is_reward_line).product_uom_qty, 25, "We should have 25 Free Large Cabinet")
        sol1.product_uom_qty = 10
        order.recompute_coupon_lines()
        self.assertEqual(order.order_line.filtered(
            lambda x: x.is_reward_line).product_uom_qty, 2, "We should have 2 Free Large Cabinet")
    ...
    def test_program_numbers_check_eligibility(self):
        ...
        self.assertEqual(order.order_line.filtered(lambda x: x.is_reward_line).product_uom_qty,
                         1, "We should receive one and only one free Large Meeting Table")
    ...
    def test_program_numbers_taxes_and_rules(self):
        ...
        p_specific_product.write(
            {'promo_code_usage': 'code_needed', 'promo_code': '20pc'})
        ...
        self.assertEqual(order.amount_total, 544.4,
                         "We should only get reduction on cabinet")
        sol1.product_uom_qty = 10
        order.recompute_coupon_lines()
        ...
        self.assertEqual(order.amount_total, 2964,
                         "The discount should be limited to $200 tax excluded")
        self.assertEqual(order.amount_untaxed, 2610,
                         "The discount should be limited to $200 tax excluded (2)")
    ...
    def test_program_numbers_one_discount_line_per_tax(self):
        ...
        (self.product_A + self.largeCabinet + self.conferenceChair +
         self.pedalBin + self.drawerBlack).write({'list_price': 100})
        (self.largeCabinet +
         self.drawerBlack).write({'taxes_id': [(4, self.tax_15pc_excl.id, False)]})
        ...
        self.assertEqual(order.amount_total, 2946.11,
                         "The order total without any programs should be 2946.11")
        self.assertEqual(order.amount_untaxed, 2576.77,
        ...
        self.assertAlmostEqual(order.amount_total, 1901.11, 2,
                               "The order total with programs should be 1901.11")
        self.assertEqual(order.amount_untaxed, 1594.95,
        ...
        self.assertEqual(order.amount_total, 1711,
                         "The order total with programs should be 1711")
        self.assertEqual(order.amount_untaxed, 1435.46,
        ...
        self.assertEqual(order.amount_total, 1619,
                         "The order total with programs should be 1619")
        self.assertEqual(order.amount_untaxed, 1355.46,
                         "The order untaxed total with programs should be 1435.45")
        ...
        self.assertEqual(len(order.order_line.ids), 9,
        ...
        order.recompute_coupon_lines()
        self.assertEqual(len(order.order_line.ids), 13,
                         "The 10% discount line should be back")
        ...
        self.assertEqual(order.amount_untaxed, 1355.46 + 81.81,
        ...
        self.assertAlmostEqual(order.amount_total, 1509.11, 2,
                               "The order total with programs should be 1509.11")
        self.assertEqual(order.amount_untaxed, 1242.22,
                         "The order untaxed total with programs should be 1242.22")
        self.assertEqual(len(order.order_line.ids), 7,
                         "Order should contains 7 lines: 4 products lines, 2 free products lines and a 20% discount line")
    ...
    def test_program_numbers_extras(self):
        ...
        self.p1.copy({'promo_code_usage': 'no_code_needed',
    ...
    def test_program_fixed_price(self):
        ...
        fixed_amount_program.discount_line_product_id.write(
        ...
        order.recompute_coupon_lines()
        self.assertEqual(order.amount_total, 176,
                         "Fixed amount discount should be totally deduced")
        self.assertEqual(len(order.order_line.ids), 2,
        ...
        self.assertEqual(len(sol2.tax_id.ids), 1,
                         "One tax should be present on the reward line")
        self.assertEqual(sol2.tax_id.id, self.tax_0pc_excl.id,
                         "The tax should be 0% Tax excl")
    ...
    def test_program_next_order(self):
        ...
        order.recompute_coupon_lines()
        self.assertEqual(len(order.order_line.ids), 1,
                         "Nothing should be added to the cart")
        self.assertEqual(len(order.generated_coupon_ids), 0,
        ...
        self.assertEqual(len(order.order_line.ids), 1,
                         "Nothing should be added to the cart (2)")
        self.assertEqual(len(generated_coupon), 1,
                         "A coupon should have been generated")
        self.assertEqual(generated_coupon.state, 'reserved',
                         "The coupon should be reserved")

        sol1.product_uom_qty = 1
        order.recompute_coupon_lines()
        generated_coupon = order.generated_coupon_ids
        self.assertEqual(len(order.order_line.ids), 1,
                         "Nothing should be added to the cart (3)")
        self.assertEqual(len(generated_coupon), 1,
                         "No more coupon should have been generated and the existing one should not have been deleted")
        self.assertEqual(generated_coupon.state, 'expired',
                         "The coupon should have been set as expired as it is no more valid since we don't have the required quantity")
        ...
        self.assertEqual(generated_coupon.state, 'reserved',
                         "The coupon should be set back to reserved as we had already an expired one, no need to create a new one")
    ...
    def test_coupon_rule_minimum_amount(self):
        ...
        self.assertEqual(order.amount_total, 165.0,
                         "The order amount is not correct")
        self.env['sale.coupon.generate'].with_context(
            active_id=self.discount_coupon_program.id).create({}).generate_coupon()
        coupon = self.discount_coupon_program.coupon_ids[0]
        self.env['sale.coupon.apply.code'].with_context(active_id=order.id).create({
            'coupon_code': coupon.code
        }).process_coupon()
        self.assertEqual(order.amount_total, 65.0,
                         "The coupon should be correctly applied")
        order.recompute_coupon_lines()
        self.assertEqual(order.amount_total, 65.0,
                         "The coupon should not be removed from the order")
    ...
    def test_coupon_and_program_discount_fixed_amount(self):
        ...
        self.assertEqual(order.amount_total, 47.5,
    ...
    def test_coupon_and_coupon_discount_fixed_amount_tax_excl(self):
        ...
        self.assertEqual(order.amount_total, 283.5,
                         "The promotion program should be correctly applied")
        ...
        self.assertEqual(order.amount_untaxed, 0.0,
                         "The untaxed amount should not go below 0")
        self.assertEqual(order.amount_total, 13.5,
        ...
        self.assertEqual(len(order.order_line), 3,
                         "The promotion program should be removed")
        self.env['sale.coupon.apply.code'].with_context(active_id=order.id).create({
            'coupon_code': coupon.code
        }).process_coupon()
        self.assertEqual(order.amount_total, 26.5,
                         "The promotion program should be correctly applied")
        order.recompute_coupon_lines()
        self.env['sale.coupon.apply.code'].with_context(active_id=order.id).create({
            'coupon_code': 'test_10pc'
    ...
    def test_coupon_and_coupon_discount_fixed_amount_tax_incl(self):
        ...
        self.assertEqual(order.amount_total, 270.0,
                         "The promotion program should be correctly applied")
        ...
        order.recompute_coupon_lines()
        # TODO fix numbers
        self.assertEqual(order.amount_total, 9.09,
    ...
    def test_program_discount_on_multiple_specific_products(self):
        ...
        self.assertEqual(order.amount_total, 87.00,
                         "Total should be 87.00, see above comment")

        # remove Drawer Black case from promotion
        p_specific_products.discount_specific_product_ids = [
        ...
        self.assertEqual(order.amount_total, 99.50,
        ...
        self.assertEqual(order.amount_total, 87.00,
                         "Total untaxed should be as per above comment")
        self.assertEqual(order.amount_untaxed, 78.34,
    ...
    def test_program_numbers_free_prod_with_min_amount_and_qty_on_same_prod(self):
        ...
        order.recompute_coupon_lines()
        self.assertEqual(len(order.order_line.ids), 2,
                         "The promotion lines should not be applied")
        sol1.write({'product_uom_qty': 3.0})
        order.recompute_coupon_lines()
        self.assertEqual(len(order.order_line.ids), 3,
                         "The promotion lines should have been added")
        self.assertEqual(order.amount_total, self.conferenceChair.lst_price * (sol1.product_uom_qty - 1) +
                         self.drawerBlack.lst_price * sol2.product_uom_qty, "The promotion line was not applied to the amount total")
        sol2.unlink()
        order.recompute_coupon_lines()
        self.assertEqual(len(order.order_line.ids), 2,
                         "The other product should not affect the promotion")
        self.assertEqual(order.amount_total, self.conferenceChair.lst_price * (
            sol1.product_uom_qty - 1), "The promotion line was not applied to the amount total")
        sol1.write({'product_uom_qty': 2.0})
        order.recompute_coupon_lines()
        self.assertEqual(len(order.order_line.ids), 1,
                         "The promotion lines should have been removed")
    ...
    def test_program_step_percentages(self):
        ...
        order.recompute_coupon_lines()
        self.assertEqual(order.amount_total, 1486.80,
        ...
        self.assertEqual(order.amount_total, 1604.8,
                         "Discount improperly applied")
        self.assertEqual(len(order.order_line.ids), 2,
                         "No discount applied while it should")
        ...
        order.recompute_coupon_lines()
        self.assertEqual(order.amount_total, 1888.0,
                         "Discount improperly applied")
        self.assertEqual(len(order.order_line.ids), 2,
                         "No discount applied while it should")
