    ...
    def _send_to_chat(self, chat_id, message):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if response.status_code == 200:
                    logger.info("Plain text message sent successfully to chat ID: %s", chat_id)
                    return True
                else:
                    logger.error("Failed to send plain text message (HTTP %s): %s", 
                                response.status_code, response.text)
                    return False
            else:
                logger.error("Failed to send message (HTTP %s): %s", 
                            response.status_code, response.text)
                return False
