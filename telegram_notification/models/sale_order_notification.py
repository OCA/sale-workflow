...
class SaleOrderNotificationLog(models.Model):
    ...
    event_type = fields.Selection([('sale_canceled', _('Sale Canceled'))], required=True)
    ...
    def send_telegram_message(self):
        telegram_bot = self.env['ir.config_parameter'].sudo().get_param('telegram_bot_token')
        ...
        if self.event_type == 'sale_canceled':
            message = _("🚨 Sales Order %s has been canceled!") % self.sale_order_id.name
