...
class SaleOrder(models.Model):
    ...
    def _send_telegram_notification(self, event_type):
        ...
        _logger.info("Preparing to send %s notification for order %s", event_type, self.name)
        ...
        if not is_enabled:
            _logger.warning("Event '%s' is not enabled in Telegram configuration", event_type)
            return False
        ...
        try:
            ...
            if success:
                _logger.info("Successfully sent %s notification for order %s", event_type, self.name)
            else:
                _logger.error("Failed to send %s notification for order %s", event_type, self.name)
                
            return success
        except Exception as e:
            _logger.exception("Error sending %s notification for order %s: %s", 
                            event_type, self.name, str(e))
            return False
