...
When a Sales Multiple is set, sales order line quantities entered in the
sale order form are proposed **rounded UP** to the nearest multiple of
the selected unit of measure. This is useful for products that must be
sold in fixed pack sizes (boxes, bundles, pallets, etc.).
...
For UoMs that share *Unit(s)* as common reference, the proposed sale
quantity is kept as an integer count of the sale order line UoM. For
dimensional UoMs, such as kilograms or meters, fractional rounded
quantities are preserved.

The rounding is intentionally limited to the sale order form.
Programmatic values written through imports, RPC calls, or custom code
are not automatically changed by this module.
...
1. Create one or more UoMs that represent the sales multiples you want
   to sell by (for example, *Pack of 100*).
...
For UoMs based on *Unit(s)*, the proposed quantity is rounded up to an
integer count of the sale order line UoM. For dimensional UoMs,
fractional quantities can be proposed.
