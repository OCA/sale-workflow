...
class AccountInvoice(InvoiceSale):
...
class AccountInvoiceLine(InvoiceSaleLine):
    ...
    def product_id_change(self, cr, uid, ids, product_id, uom_id, *args, **kwargs):
        ...
        res = super(AccountInvoiceLine, self).product_id_change(
            cr, uid, ids, product_id, uom_id, *args, **kwargs)
        if product_id:
            if len(args) >= 3:
                invoice_type = args[2]
            else:
            ...
            if context is None:
                context = {}
            if context.get('tax_inc') and \
                    invoice_type in ('out_invoice', 'out_refund'):
                res['value'].update({
                    'price_unit': product.list_price_tax_inc,
                    })
        return res
