...
class SaleOrderLine(models.Model):
    ...
    def product_id_change_with_wh(
        ...
        line_obj = self.pool.get('sale.order.line').browse(cr, uid, ids)
        location_obj = self.pool.get('stock.location').browse(
            cr, uid, line_obj.order_id.warehouse_id.lot_stock_id.id)
        prod_obj = self.pool.get('product.product').browse(
            cr, uid, product)
        quant_ids = self.pool.get('stock.quant').quants_get_prefered_domain(
            cr, uid, location=location_obj,
            product=prod_obj,
            qty=qty,
            restrict_lot_id=line_obj.lot_id.id, context=context)
        lots = []
