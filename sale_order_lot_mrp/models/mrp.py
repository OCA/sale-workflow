...
class MrpProduction(models.Model):
    ...
    def _workorders_create(self, bom, bom_data):
        ...
        for workorder in workorders:
            if (
                workorder.product_id.tracking != "none"
                and workorder.product_id.auto_generate_prodlot
            ):
                workorder.final_lot_id = workorder.production_id.lot_id
            for mv_lot in workorder.active_move_lot_ids:
                if (
                    mv_lot.product_id.tracking != "none"
                    and mv_lot.product_id.auto_generate_prodlot
                    and workorder.final_lot_id
                ):
                    idx += 1
