...
class TestPurchaseSaleStockAutoCancel(BaseCommon):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        new_test_user(
            cls.env,
            login="sale_purchase_stock_auto_cancel_user",
    ...
    @users("sale_purchase_stock_auto_cancel_user")
    def test_purchase_auto_cancel_with_mto(self):
        """Test that the purchase order is auto-cancel when the sale order
        is canceled for a product with MTO route
        """
        self.company.purchase_auto_cancel = True
    ...
    @users("sale_purchase_stock_auto_cancel_user")
    def test_purchase_no_auto_cancel(self):
        """Test that the purchase order is not auto-cancel when the sale order
        is canceled for a product with MTO route
        """
        self.company.purchase_auto_cancel = False
        self.sale_order.action_confirm()
        purchase = self.sale_order._get_purchase_orders()
        self.assertEqual(purchase.state, "draft")
        self.assertEqual(len(purchase.order_line), 2)
