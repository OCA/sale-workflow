You can find the following configurations in the settings:

1.  **Subscriptions Backward Days**: Backward days to search documents
    to update subscriptions.
2.  **Sale Planner Forward Months**: Forward months to create calendar
    events.
3.  **Send invitation to attendees**: Send invitations to attendees when
    a planner event is created.
4.  **Sale planner order cut hour**: Time of the next day until which
    orders of the current day are assigned.
5.  **Calendar event max duration**: Show a warning message when
    duration is more than this time. Set 00:00 to disable warning.

Other setting are available with system parameters

1.  **Sale order partner** when a so is created from a event planned.
    You can create or update the system parameter
    **sale_planner_calendar.create_so_to_commercial_partner** with True
    value to create the sale order to commercial partner instead of
    partner
2.  **Sale order salesperson** when a so is created from an event
    planned. By default the salesperson assigned to the event (usually
    whoever is doing the visit) is kept on the new order - typical when
    a salesperson covers for another one's vacation. You can create or
    update the system parameter
    **sale_planner_calendar.keep_partner_salesperson** with True value
    to keep the partner's own assigned salesperson on the new order
    instead
