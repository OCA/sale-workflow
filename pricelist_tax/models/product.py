...
HELP = _("Only price items with 'Based on' field set to "
...
class ProductPricelist(models.Model):
    ...
    price_include_taxes = fields.Boolean(
        string="Price include taxes", default=True,
    ...
    def name_get(self):
        ...
        suffix = {x.id: _(' (tax include)')
                  for x in pricelists if x.price_include_taxes}
        names = []
        for elm in res:
    ...
    def _constrains_price_item_price_include(self):
        for rec in self:
            ...
            if rec.base in ('pricelist', 'standard') and price_incl_tax:
                    raise UserError(HELP + _(
                        "You encoded 'Based on' field to %s" % rec.base))
