        ...
        for order in self:
            ...
            for tax_dict in order.tax_totals["groups_by_subtotal"].values():
                for tax in tax_dict:
                    ...
                    tax["tax_group_amount"] -= amount_discount_by_tax.get(
                        tax["tax_group_id"], 0.0
                    )
                    tax["formatted_tax_group_amount"] = formatLang(
                        self.env, tax["tax_group_amount"], currency_obj=currency
                    )
