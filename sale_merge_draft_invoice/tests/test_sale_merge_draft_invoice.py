...
class TestSaleMergeDraftInvoice(TransactionCase):
    def setUp(self):
        ...
        # users
        self.user1_id = self._create_user("user_1", [self.group_sale], self.company1)
        self.user2_id = self._create_user(
            "user_2",
            [self.group_sale_merge_draft_invoice, self.group_sale],
            self.company1,
        )
    ...
    def test_create_invoice_case_1(self):
        ...
        self.assertEqual(
            self.company1.sale_merge_draft_invoice,
            True,
        ...
        self.assertEqual(inv1.state, "draft", "The invoice status should be 'draft'")
        so2 = self._create_sale_order(self.user1_id)
        inv2 = self._create_invoice_from_sale(so2, self.user1_id)
        self.assertEqual(inv2.state, "draft", "The invoice status should be 'draft'")
        # The result is one invoice with all the lines
        self.assertEqual(inv1, inv2, "The invoices should be the same.")
