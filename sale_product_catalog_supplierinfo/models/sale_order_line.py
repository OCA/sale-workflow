...
class SaleOrderLine(models.Model):
    ...
    def _onchange_vendor_id_clear_mismatched_supplierinfo(self):
        """Drop a stale pin left over from a different vendor.

        ``supplierinfo_id`` is defaulted from the catalog card that was
        clicked (see ``sale_order._update_order_line_info()`` and the
        ``default_supplierinfo_id`` seeded on the manual "Add new order
        line" wizard's ``vendor_id`` domain). Manually picking a different
        vendor afterwards must not leave that pin pointing at the old
        vendor's row - it would silently mis-price/mis-purchase against a
        supplierinfo of a partner that no longer matches the line.
        """
        if self.supplierinfo_id and self.supplierinfo_id.partner_id != self.vendor_id:
