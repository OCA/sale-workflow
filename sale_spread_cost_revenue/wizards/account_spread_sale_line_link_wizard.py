...
class AccountSpreadSaleLineLinkWizard(models.TransientModel):
    _name = "account.spread.sale.line.link.wizard"
    _description = "Account Spread Sale Line Link Wizard"
    ...
    sale_line_id = fields.Many2one(
        "sale.order.line", readonly=True, required=True, ondelete="cascade"
    )
    sale_id = fields.Many2one(related="sale_line_id.order_id", readonly=True)
    ...
    def _compute_invoice_type(self):
        for wizard in self:
            wizard.invoice_type = "out_invoice"
            wizard.spread_type = "sale"

    @api.depends("invoice_type", "company_id")
    def _compute_spread_journal(self):
        for wizard in self:
            wizard.spread_journal_id = (
                wizard.company_id.default_spread_revenue_journal_id
            )

    @api.depends("invoice_type", "company_id")
    def _compute_spread_account(self):
        for wizard in self:
            wizard.spread_account_id = (
                wizard.company_id.default_spread_revenue_account_id
            )
    ...
    def _onchange_user_invoice_line_account(self):
        """For sales order, this means use product's account"""
        product = self.sale_line_id.product_id
        account = (
            product.property_account_income_id
            or product.categ_id.property_account_income_categ_id
        )
        self.spread_account_id = self.use_invoice_line_account and account or False
    ...
    def confirm(self):
        ...
        if self.spread_action_type == "link":
            if not self.sale_line_id.spread_id:
                if self.spread_id.sale_line_id:
                    raise UserError(_("Cannot link with already linked account.spread"))
                self.sale_line_id.spread_id = self.spread_id
                self.spread_id.sale_line_id = self.sale_line_id
            return {
                ...
                "target": "current",
                "readonly": False,
                "res_id": self.sale_line_id.spread_id.id,
            }
        elif self.spread_action_type == "new":
            debit_account = self.spread_account_id
            credit_account = self.exp_rev_account_id or product_account
            date_order = self.sale_id.date_order or fields.Date.today()
            return {
                ...
                "readonly": False,
                "context": {
                    "default_name": self.sale_line_id.name,
                    ...
                    "default_spread_date": date_order,
                },
            }
            ...
            if not self.sale_line_id.spread_id:
                ...
                if self.template_id.use_invoice_line_account:
                    ...
                    spread_account_id = product_account.id

                spread_vals = self.template_id._prepare_spread_from_template(
                    spread_account_id=spread_account_id
                )
                spread_vals["sale_line_id"] = self.sale_line_id.id  # Assign ID
                date_order = self.sale_id.date_order
                date_order = date_order or self.template_id.start_date
                date_order = date_order or fields.Date.today()
                spread_vals["spread_date"] = date_order
                spread_vals["name"] = ("%s %s") % (
                    spread_vals["name"],
                    self.sale_line_id.name,
                )
                spread_vals["credit_account_id"] = account.id
                spread_vals["currency_id"] = self.sale_id.currency_id.id
                self.env["account.spread"].create(spread_vals)
            return {
                "name": _("Spread Details"),
                "view_mode": "form",
                "res_model": "account.spread",
