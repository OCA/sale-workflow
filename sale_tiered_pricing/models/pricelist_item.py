...
class ProductPricelistItem(models.Model):
    """Add the new tiered and volume pricing ways to compute_price,
    ...
    compute_price = fields.Selection(
        selection_add=[("tier", "Tiered pricing"), ("volume", "Volume pricing")]
    )
    tiered_pricelist_id = fields.Many2one(
        "product.pricelist",
        domain="[('is_tiered_pricing', '!=', False)]",
        help="In case of tiered or volume pricing, the pricing that will be used.",
    )

    @api.constrains("compute_price")
    def _constrains_tier_items(self):
        tier_items = self.filtered(lambda i: i.compute_price in ["tier", "volume"])
        bad_tier_items = tier_items.filtered(
            lambda i: not i.tiered_pricelist_id.is_tiered_pricing
        )
        if bad_tier_items:
            message = _("Tiered pricings should be set on all tiered and volume items.")
            raise ValidationError(message)

    def _get_pricelist_item_name_price(self):
        super(ProductPricelistItem, self)._get_pricelist_item_name_price()
        for item in self.filtered(lambda i: i.compute_price in ["tier", "volume"]):
            item.price = item.tiered_pricelist_id.name

    def _compute_price(self, price, price_uom, product, quantity=1.0, partner=False):
        if self.compute_price in ["tier", "volume"]:
            ...
            if self.compute_price == "tier":  # the unit price is the weighted average
                price = sum(q * p for q, p in qps) / quantity
            else:  # the unit price is the last tier price
                price = qps[-1][1]
