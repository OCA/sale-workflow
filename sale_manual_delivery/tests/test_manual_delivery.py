...
class TestSaleStock(TestSale):
    ...
    def test_00_sale_manual_delivery(self):
        ...
        wizard = self.env['manual.delivery'].with_context(
            {'active_model': 'sale.order.line',
             'active_ids': self.so.order_line.ids}).create({})
        ...
        self.assertTrue(self.so.picking_ids, 'Sale Manual Delivery: picking \
        ...
        wiz_act['res_id'] = wizard.id
        wiz = self.env[wiz_act['res_model']].browse(wiz_act['res_id'])
        self.assertFalse(wiz.line_ids, 'Sale Manual Delivery: After picking \
            creation for all products, no lines should be left in the wizard')
        wiz.record_picking()
        self.assertEqual(len(self.so.picking_ids), 1.0, 'Sale Manual \
            Delivery: picking number should be 1.0 instead of %s as all qty \
            has been already created in a picking' % len(self.so.picking_ids))
        ...
        self.assertEqual(del_qty, 5.0, 'Sale Stock: delivered quantity should \
            be 5.0 instead of %s after complete delivery' % del_qty)
    ...
    def test_01_sale_standard_delivery(self):
        ...
        # confirm our standard so, check the picking
        self.so.action_confirm()
        self.assertTrue(self.so.picking_ids, 'Sale Manual Delivery: \
    ...
    def test_02_sale_various_manual_delivery(self):
        ...
        # Check quantity delivered
        del_qty = sum(sol.qty_delivered for sol in self.so.order_line)
        self.assertEqual(del_qty, 2.0, 'Sale Stock: delivered quantity should \
            be 2.0 instead of %s after complete delivery' % del_qty)
        ...
        wiz.record_picking()
        self.assertEqual(len(self.so.picking_ids), 2.0, 'Sale Manual \
    ...
    def test_03_sale_selected_lines(self):
        ...
        all_lines = self.so1.order_line + self.so2.order_line + \
            self.so3.order_line
        # create a manual delivery for part of ordered quantity
        wizard = self.env['manual.delivery'].with_context(
            {'active_model': 'sale.order.line',
             'active_ids': some_lines.ids}).create({})
        wizard.onchange_line_ids()
        wiz_act = self.env.ref(
            'sale_manual_delivery.action_wizard_manual_delivery'
        ).read()[0]
        ...
        wiz.record_picking()
        # check picking is created
        self.assertTrue(self.so1.picking_ids, 'Sale Manual Delivery: picking \
            should be created after "manual delivery" wizard call')
        self.assertFalse(self.so2.picking_ids, 'Sale Manual Delivery: picking \
        ...
        undelivered = self.env['sale.order.line'].search(
            [('pending_qty_to_deliver', '=', True),
             ('state', '=', 'sale')]).filtered(
            lambda s: s.id in all_lines.ids)
        self.assertEqual(undelivered, self.so2.order_line,
    ...
    def test_04_sale_single_picking(self):
        ...
        so_vals = {
            ...
            'order_line': [(0, 0, {
                ...
                'product_uom_qty': 1.0,
                'product_uom': self.product.uom_id.id,
                'price_unit': self.product.list_price}), (0, 0, {
                    'name': self.product2.name,
                    'product_id': self.product2.id,
                    'product_uom_qty': 2.0,
                    'product_uom': self.product2.uom_id.id,
        ...
        wizard = self.env['manual.delivery'].with_context(
            {'active_model': 'sale.order.line',
             'active_ids': self.so1.order_line.ids}).create({})
        wizard.onchange_line_ids()
        wiz_act = self.env.ref(
            'sale_manual_delivery.action_wizard_manual_delivery'
        ).read()[0]
        ...
        wiz = self.env[wiz_act['res_model']].browse(wiz_act['res_id'])
        wiz.record_picking()
        self.assertEqual(len(self.so1.picking_ids), 1.0, 'Sale Manual \
            Delivery: picking number should be 1.0 instead of %s as created\
            at once' % len(self.so1.picking_ids))
