        ...
        for line in self:
            if line.rental_type == "rental_extension":
                ...
                if line.rental_qty != line.extension_rental_id.rental_qty:
                    raise ValidationError(
                        _(
                            f"On the sale order line with rental service "
                            f"{line.product_id.display_name}, "
                            f"you are trying to extend a rental with a rental "
                            f"quantity {line.rental_qty} that is different "
                            f"from the quantity of the original rental "
                            f"{line.extension_rental_id.rental_qty}. "
            ...
            if line.rental_type in ("new_rental", "rental_extension"):
                if not line.product_id.rented_product_id:
                    raise ValidationError(
                        _(
                            f"On the 'new rental' sale order line with product "
                            f"'{line.product_id.display_name}', "
                            f"we should have a rental service product !"
                        )
                    )
                if line.product_uom_qty != line.rental_qty * line.number_of_days:
                    raise ValidationError(
                        _(
                            f"On the sale order line with product "
                            f"'{line.product_id.display_name}' "
                            f"the Product Quantity ({line.product_uom_qty}) "
                            f"should be the "
                            f"number of days ({line.number_of_days}) "
                            f"multiplied by the Rental Quantity ({line.rental_qty})."
                        )
                    )
                ...
                if line.product_uom_qty != line.sell_rental_id.rental_qty:
                    raise ValidationError(
                        _(
                            f"On the sale order line with product "
                            f"{line.product_id.display_name} "
                            f"you are trying to sell a rented product with a "
                            f"quantity ({line.product_uom_qty}) that is different from"
                            f" the rented quantity ({line.sell_rental_id.rental_qty}). "
                            f"This is not supported."
                        )
                    )
                ...
                if not self.rental_type:
                    ...
                    if compare_qty == -1:
                        res["warning"] = {
                            ...
                            "message": _(
                                f"You want to rent {self.rental_qty:.2f} "
                                f"{product_uom.name} but you only "
                                f"have {in_location_available_qty:.2f} "
                                f"{product_uom.name} currently available on the "
                                f"stock location '{rental_in_location.name}' "
                                f"! Make sure that you "
                                f"get some units back in the mean time or "
                                f"re-supply the stock location "
                                f"'{rental_in_location.name}'."
                            ),
                        }
