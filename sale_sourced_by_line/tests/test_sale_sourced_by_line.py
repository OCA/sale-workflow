...
class TestSaleSourcedByLine(BaseCommon):
    ...
    def test_sales_order_multi_source(self):
        ...
        references = order.order_line.stock_reference_id
        self.assertEqual(len(references), 2)
        self.assertIn(f"{order.name}/{self.warehouse1.name}", references.mapped("name"))
    ...
    def test_sales_order_no_source(self):
        ...
        self.assertEqual(len(order.order_line.stock_reference_id), 1)
