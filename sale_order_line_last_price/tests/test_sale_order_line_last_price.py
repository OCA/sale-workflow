...
class TestSaleOrderLinePriceHistory(SavepointCase):
    ...
    def test_onchange_partner_id_include_quotations(self):
        ...
        self.sale_order_5 = self.env["sale.order"].create(
            {"partner_id": self.partner_2.id}
        )
        self.sale_order_line_5 = self.env["sale.order.line"].create(
            {
                "order_id": self.sale_order_4.id,
                "name": self.product.name,
                "product_id": self.product.id,
        ...
        wizard = self.launch_wizard(self.sale_order_line_5.id)
