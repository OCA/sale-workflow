...
class SaleOrderLine(models.Model):
    ...
    unit_weight = fields.Float(
        string="Unit Weight", digits=dp.get_precision("Stock Weight"),
    )
    total_ordered_weight = fields.Float(
        compute="_compute_total_ordered_weight", string="Total Ordered Weight",
    )
    total_delivered_weight = fields.Float(
        compute="_compute_total_delivered_weight", string="Total Delivered Weight",
    )
